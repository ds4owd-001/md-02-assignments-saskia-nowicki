...
ggplot(gapminder_2007,
       aes(continent, lifeExp, fill = continent)) +
  geom_boxplot(outlier.shape = NA)
str(gapminder_2007)
#Continent is categorical, life expectancy is continuous.
```
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: it takes a specified categorical variable from a tibble, each level of the variable is listed across the x-axis and the counts of the different levels of the variable are calculated for the y-axis.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function: The histogram function counts the number of observations that fall into 'bins', or sub-ranges, of a continuous variable. It does for continuous data what the bar plot does for categorical data.

7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: the first number specifies the lower limit for the x-axis, the second specifies the upper limit for the x-axis, the third specifies the magnitude of each 'bin'.
...
ggplot(gapminder_2007,
       aes(x = lifeExp, fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(30,100,10))
```
